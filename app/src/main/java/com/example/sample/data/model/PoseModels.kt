...
fun DetectedPose.torsoCenter(): PosePoint? {
    val torso = listOfNotNull(
        landmarks[HeroPoseLandmarks.LEFT_SHOULDER],
        landmarks[HeroPoseLandmarks.RIGHT_SHOULDER],
        landmarks[HeroPoseLandmarks.LEFT_HIP],
        landmarks[HeroPoseLandmarks.RIGHT_HIP],
    )
    if (torso.isEmpty()) return null

    return PosePoint(
        x = torso.sumOf { it.x.toDouble() }.toFloat() / torso.size,
        y = torso.sumOf { it.y.toDouble() }.toFloat() / torso.size,
        confidence = torso.sumOf { it.confidence.toDouble() }.toFloat() / torso.size,
    )
}
